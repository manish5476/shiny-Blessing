...
exports.logout = exports.updateUserPassword = exports.resetPassword = exports.forgotPassword = exports.restrictTo = exports.protect = exports.login = exports.signup = void 0;
...
const logger = winston_1.default.createLogger({
    ...
    transports: [
        new winston_1.default.transports.File({ filename: 'logs/auth.log', level: 'info' }),
    ]
});
...
const signToken = (id) => {
    if (!process.env.JWT_SECRET) {
        ...
        throw new appError_1.default('Server configuration error: JWT secret not found.', 500);
    }
...
const createSendToken = (user, statusCode, res) => {
    const token = signToken(user._id); // Explicitly cast _id to ObjectId
    res.cookie('jwt', token, cookieOptions);
    // Remove password from the user object sent in the response
    user.password = undefined; // Type assertion needed for strict mode
    const userData = {
        _id: user._id,
        name: user.name,
        email: user.email,
...
exports.signup = (0, catchAsyncModule_1.catchAsync)(async (req, res, next) => {
...
exports.login = (0, catchAsyncModule_1.catchAsync)(async (req, res, next) => {
...
exports.protect = (0, catchAsyncModule_1.catchAsync)(async (req, res, next) => {
    let token;
    ...
    if (!process.env.JWT_SECRET) {
        logger.error('JWT_SECRET environment variable is not defined!');
        return next(new appError_1.default('Server configuration error: JWT secret not found.', 500));
    }
    let decoded;
    try {
        ...
        decoded = await new Promise((resolve, reject) => {
            jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET, (err, result) => {
                ...
                if (typeof result === 'string' || !result) {
                    return reject(new jsonwebtoken_1.default.JsonWebTokenError('Invalid token: expected object payload.'));
                }
                resolve(result); // Assert to our CustomJwtPayload
            });
        });
    }
    catch (err) {
        ...
        return next(new appError_1.default('Authentication error. Please try again.', 500));
    }
    ...
    if (currentUser.changedPasswordAfter(decoded.iat)) { // `iat` is guaranteed by JwtPayload but `!` asserts it's not undefined
        return next(new appError_1.default('Password changed recently. Please log in again.', 401));
    }
...
const restrictTo = (...roles) => {
    return (req, res, next) => {
        if (!req.user) {
            return next(new appError_1.default('Authentication required to check roles.', 401));
        }
...
// --- Password Reset ---
exports.forgotPassword = (0, catchAsyncModule_1.catchAsync)(async (req, res, next) => {
    const user = await UserModel_1.default.findOne({ email: req.body.email });
    if (!user) {
        return next(new appError_1.default('There is no user with this email address.', 404));
    }
...
exports.resetPassword = (0, catchAsyncModule_1.catchAsync)(async (req, res, next) => {
...
exports.updateUserPassword = (0, catchAsyncModule_1.catchAsync)(async (req, res, next) => {
    if (!req.user) {
        return next(new appError_1.default('Authentication required to update password.', 401));
    }
    const user = await UserModel_1.default.findById(req.user.id).select('+password'); // Use req.user.id (virtual) or req.user._id
    if (!user) {
        return next(new appError_1.default('User not found. Please log in again.', 404));
    }
