...
orderSchema.pre('save', async function (next) {
    ...
    const productsToUpdate = []; // Declare at correct scope
    try {
        session = await mongoose_1.default.startSession();
        session.startTransaction();
        for (const item of doc.products) {
            // item.product is already Types.ObjectId as per IOrderProduct interface
            const product = await productModel_1.default.findById(item.product).session(session);
            if (!product) {
        ...
        for (const product of productsToUpdate) {
            ...
            const originalOrderItem = doc.products.find(item => item.product.toString() === product._id.toString());
